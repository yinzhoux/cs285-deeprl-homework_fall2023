...
\end{table}  
\problem{3.2}
Varing \textbf{training batch size} from 100 to 1000 with step size 100, we get the performance vs. batch\_size graph[\ref{fig:bc_trend}] of environment \textbf{Hopper-v4}.
\begin{table}[H]
    \centering
    \begin{tabularx}{\textwidth}{lXXXX}
        \toprule
...
\begin{proof}
    ...
    \begin{enumerate}
        \item $X_n = 0(Pr = 1-\alpha)$:
            The policy imedietely switch to $\widetilde{\pi}^{n-1}$ with $C_{t, n-1}$. By our assumption, the cost is
            $$
            \mathbb{E}[C_{t, n} | X_n = 0] = (1-\alpha)\times A(t, n-1)
            $$
        \item $X_n \geq1(Pr = \alpha)$:
            The policy gives action to the first step with $\epsilon$ probability of failing. 
            
            If the policy fails the first step, the whole trajectory's error is up to 
            $$
        ...
        $$
        Let $T$ equals to $t$, we concluded:
        $$
...
\begin{proof}
    ...
    $
    C(\pi) \leq 
    \Sigma_{t=1}^T max\mathbb{E}_{s_t \sim p_{\pi}}Pr[\pi(s_t) \ne\pi^{\star}(s_t)]
    \leq T
    $.
    $\pi^n$ is policy that transfer control from $\widetilde{\pi}^{n}$ to expert policy 
...
$$
Let $\alpha = 1/T$ and $N = T log(1/\epsilon)$.Substituding into the formula:
$$
