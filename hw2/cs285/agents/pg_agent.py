...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        obs: np.ndarray = np.concatenate(obs)
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [self._discounted_return(one_trajectory_rewards) for one_trajectory_rewards in rewards]
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # TODO: use the helper function self._discounted_reward_to_go to calculate the Q-values
            q_values = [self._discounted_reward_to_go(one_trajectory_rewards) for one_trajectory_rewards in rewards]
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        T = len(rewards)
        Return = 0
        reward_to_go = [0.0] * (T+1) 
        for i in range(T):
            reward_to_go[T - i - 1] = reward_to_go[T-i] * self.gamma + rewards[T - i -  1]
